...
class Motor:
    ...
    def safe_write1(self, addr, value):
        result, error = packetHandler.write1ByteTxRx(portHandler, self.id, addr, value)
        return result == COMM_SUCCESS and error == 0

    def safe_write4(self, addr, value):
        value = ctypes.c_uint32(value).value
        result, error = packetHandler.write4ByteTxRx(portHandler, self.id, addr, value)
        return result == COMM_SUCCESS and error == 0

    def safe_read2(self, addr):
        val, result, error = packetHandler.read2ByteTxRx(portHandler, self.id, addr)
        if result != COMM_SUCCESS or error:
            return None
        return to_signed16(val)

    def safe_read4(self, addr):
        val, result, error = packetHandler.read4ByteTxRx(portHandler, self.id, addr)
        if result != COMM_SUCCESS or error:
            return None
    ...
    def safe_write2(self, addr, value):
        ...
        result, error = packetHandler.write2ByteTxRx(portHandler, self.id, addr, value)
        return result == COMM_SUCCESS and error == 0

    def set_operating_mode(self, mode):
        result, error = packetHandler.write1ByteTxRx(portHandler, self.id, ADDR_OPERATING_MODE, mode)
        return result == COMM_SUCCESS and error == 0
...
for m in motors:
    # Extended position mode allows signed multi-turn goals (negative direction included).
    m.disable()
    if not m.set_operating_mode(OPERATING_MODE_EXTENDED_POSITION):
        print(f"⚠️ Failed to set Extended Position Mode on ID {m.id}")
    else:
        print(f"ID {m.id}: Extended Position Mode enabled")
    m.enable()

# ===== CONTROL PARAMS =====
TARGET_TORQUE = 0.4         # target torque for calibration (Nm)
TORQUE_TOLERANCE = 0.05      # acceptable deviation from target (Nm)
STEP_TURNS = -0.05           # constant movement step during calibration (turns per loop)
POST_MOVE_SETTLE_S = 0.3     # wait for motor to come to complete standstill before measuring (increased)
CONTROL_DT_S = 0.05          # main control-loop period
DISPLAY_ALPHA = 0.2          # smoothing for display only
...
if initial_position is None:
    position_goal = 0.0
    print("⚠️ Could not read initial position, using 0.0 turns")
else:
...
try:
    ...
    # Initialize goal from current motor position
    initial_position = motors[0].get_position()
    if initial_position is None:
        position_goal = 0.0
        print("⚠️ Could not read initial position, using 0.0 turns")
        ...
        position_goal = max(MIN_POSITION, min(MAX_POSITION, initial_position))
        print(f"Initial position: {position_goal:.3f} turns")
        print(f"Starting CALIBRATION phase - seeking target torque of {TARGET_TORQUE:.3f} Nm...\n")
    ...
    while True:
        for m in motors:
            ...
            if current_readings:
                current_mA = sum(current_readings) / len(current_readings)
                raw_current = int(current_mA / CURRENT_UNIT)  # For back-drive check
            else:
                current_mA = None
                raw_current = None
            ...
            if torque is not None:
                if filtered_torque is None:
                    ...
                    filtered_torque = DISPLAY_ALPHA * torque + (1.0 - DISPLAY_ALPHA) * filtered_torque
            ...
            if torque is None and not warned_torque_unavailable:
                print("⚠️ Torque read unavailable")
            ...
            tqf_text = f"{filtered_torque:6.3f}" if filtered_torque is not None else "   N/A"
            phase_text = phase
            ...
            print(
                ...
                f"I={mA_text} mA | torque={tq_text} Nm (filtered={tqf_text} Nm)",
            )
        ...
        if position_goal <= MIN_POSITION and phase == "CALIBRATION":
            print("\n⚠️ Reached minimum position without finding target torque!")
            break
